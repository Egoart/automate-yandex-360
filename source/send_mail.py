    ...
    try:
        with smtplib.SMTP("smtp.yandex.ru", 587) as s:
            ...
            msg = (
                ...
                f"ВНИМАНИЕ! Пароль необходимо сменить на свой персональный. После первого входа в почтовый ящик (на Яндексе) с использованием временного пароля, система запросит ввод своего собственного пароля. Рекомендую записать введенный пароль отдельно, чтобы не потерять доступ к почте. Также рекомендую периодически менять пароль (изображение шестеренки (вверху справа в основном окне почты, там где список писем) -> 'Все настройки' -> 'Безопасность' -> 'менять пароль').\n\n\n"
